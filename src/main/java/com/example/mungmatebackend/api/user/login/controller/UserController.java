...
public class UserController {
    ...
    @Operation(summary = "유저 프로필 업데이트(수정 가능 여부 확인) API")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "유저 업데이트 가능",
                    content = @Content(schema = @Schema(implementation = UserDto.updateCheckProfileResponse.class))
            ),
            @ApiResponse(
                    responseCode = "404",
                    description = "존재 하지 않는 유저",
    ...
    @GetMapping("/{userId}/check-profile")
    public ResponseEntity<UserDto.updateCheckProfileResponse> updateCheckProfile(
            @Parameter(name = "userId", description = "유저 id")
            @PathVariable Long userId
    ) {
